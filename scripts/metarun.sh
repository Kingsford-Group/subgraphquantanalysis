...
# download data
${script_folder}/../data/getdata_hubmap.sh ${out_folder}/Reads_hubmap/
...
while read -r line; do
	...
	python ${script_folder}/../src/process.py ${out_folder}/Reads_hubmap/${ID} ${SalmonIndex} ${OutPrefix}/${ID} ${OutPrefix}/${ID}/graphsalmon ${GTFfile} ${GenomeFasta}
...
#################### MCF10 dataset ####################
# download data
${script_folder}/../data/getdata_mcf10.sh ${out_folder}/Reads_mcf10/
...
# estimate flow for MCF10 dataset
while read -r line; do
	...
	python ${script_folder}/../src/process.py ${out_folder}/Reads_mcf10/${id} ${SalmonIndex} ${OutPrefix}/${id}/ ${OutPrefix}/${id}/graphsalmon ${GTFfile} ${GenomeFasta}

done < ${script_folder}/../data/Metadata_mcf10.txt

#################### Immune cell dataset ####################
# download data
${script_folder}/../data/getdata_immune29.sh ${out_folder}/Reads_immune29

GTFfile="${out_folder}/gencode.v26.annotation.gtf"
GenomeFasta="${out_folder}/GRCh38.p10.genome.fa"
SalmonIndex="${out_folder}/gencode.v26.full"
OutPrefix="${out_folder}/Immune29"

# estimate flow for Immune cell dataset
while read -r line; do
	...
	python ${script_folder}/../src/process.py ${out_folder}/Reads_immune29/${id} ${SalmonIndex} ${OutPrefix}/${id}/ ${OutPrefix}/${id}/graphsalmon ${GTFfile} ${GenomeFasta}

done < ${script_folder}/../data/Metadata_immune29.txt
